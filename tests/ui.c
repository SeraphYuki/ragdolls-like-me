...
static void CreateFrameBuffer(UI *ui){
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ui->viewport.w, ui->viewport.h, 0, 
...
void UI_Init(UI *ui, int w, int h){
	...
	ui->viewport.w = w;
	ui->viewport.h = h;
	ui->test = ImageLoader_CreateImage("Resources/tex.png", 1);
...
void UI_RenderRectTex(UI *ui, float x, float y, u16 w, u16 h, u16 ix, u16 iy, u16 iw, u16 ih, u8 r, u8 g, u8 b, u8 a){
	...
	FontRenderer_RenderString(&ui->fr, x, y, "testing",255,255,255,255);
	...
	for(k = 0; k < 12; k+=2){
		pos[0] = x + ((s16)RectTriangleVerts[k] * w);
		pos[1] = y + ((s16)RectTriangleVerts[k+1] * h);
		coord[0] = (ix + ((s16)RectTriangleVerts[k] * iw)) * 1.0f/ui->test.w;
		coord[1] = 1-((iy + ((s16)RectTriangleVerts[k+1] *ih)) * 1.0f/ui->test.h);
		glBindBuffer(GL_ARRAY_BUFFER, ui->posVbo);
		glBufferSubData(GL_ARRAY_BUFFER, offset*sizeof(pos), sizeof(pos), pos);
		glBindBuffer(GL_ARRAY_BUFFER, ui->uvVbo);
	...
	glBindTexture(GL_TEXTURE_2D, ui->test.glTexture);
...
void UI_RenderRectLines(UI *ui, float x, float y, u16 w, u16 h, u8 r, u8 g, u8 b, u8 a){
	...
	//Shaders_UseProgram(TEXTURELESS_2D_SHADER);
	//glUniform4f(Shaders_GetUniformColorLocation(), r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f); 
	//glUniform2f(Shaders_GetInvViewportLocation(), 1.0f/ui->viewport.w, 1.0f/ui->viewport.h); 
	...
	//glUniform4f(ui->shaders[THOTH_TEXTURELESS_SHADER].uniColorLoc, 1, 1, 1, 1); 
}
