...
static void BoneUpdateVelocities(Bone *bone,Vec4 *matrices){
    ...
    pos = Math_Vec3AddVec3(bone->pos,bone->linVel);
    rot = Math_QuatMult(bone->rot,Math_EulerToQuat(bone->angVel));
    ...
    if(bone->parent){
        Math_MatrixMatrixMult(bone->absMatrix, bone->parent->absMatrix, bone->absMatrix); 
        bone->absPos = Math_Vec3AddVec3(bone->parent->absPos,Math_QuatRotate(bone->parent->absRot,pos));
        bone->absRot = Math_QuatMult(bone->parent->absRot, rot);
...
void Skeleton_Apply(Skeleton *skeleton){
    ...
    BoneUpdateVelocities(skeleton->root, skeleton->matrices);
...
static void LoadModel(Model *model, FILE *fp, u16 stride){
    ...
    for(k = 0; k < model->nMaterials; k++){
        ...
        model->materials[k].diffuse = 
        Math_Vec4MultFloat(model->materials[k].diffuse, 1);
    }
    ...
    Deflate_Read(fp, elements, sizeof(u32) * totalElements);
...
static void LoadSkeleton(Skeleton *skeleton, FILE *fp){
    ...
    for(k = 0; k < skeleton->nBones; k++){
        ...
        if(parentIndex >= 0){
            ...
            if(skeleton->root){
                bone->parent = NULL;
                if(skeleton->root->nChildren < BONE_MAX_CHILDREN)
                    skeleton->root->children[skeleton->root->nChildren++] = bone;
