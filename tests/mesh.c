...
static void BoneUpdate(Bone *bone, PlayingAnimation *anims, int nAnims, Vec4 *matrices){
	...
	if(bone->parent){
	     Math_MatrixMatrixMult(bone->absMatrix, bone->parent->absMatrix, bone->absMatrix); 
		bone->absRot = Math_QuatMult(bone->parent->absRot, bone->absRot);
		bone->absPos = Math_Vec3AddVec3(bone->parent->absPos, Math_QuatRotate(bone->parent->absRot,bone->absPos));
	}

    Math_MatrixMatrixMult(matrix, bone->absMatrix, bone->invBindMatrix);

    bone->axes[0] = Math_Vec3Normalize(*((Vec3*)&bone->absMatrix[0]));
	  bone->axes[1] = Math_Vec3Normalize(*((Vec3*)&bone->absMatrix[4]));
	  bone->axes[2] = Math_Vec3Normalize(*((Vec3*)&bone->absMatrix[9]));
