...
void Math_Matrix6x6Set(float *m, float *m1, float *m2, float *m3, float *m4){
	  memcpy(&m[0], m1, sizeof(float)*3);
	  memcpy(&m[3], m2, sizeof(float)*3);
	  memcpy(&m[6], m3, sizeof(float)*3);
	  memcpy(&m[9], m4, sizeof(float)*3);
	  memcpy(&m[12], &m1[3], sizeof(float)*3);
	  memcpy(&m[15], &m2[3], sizeof(float)*3);
	  memcpy(&m[18], &m3[3], sizeof(float)*3);
	  memcpy(&m[21], &m4[3], sizeof(float)*3);
	  memcpy(&m[24], &m1[6], sizeof(float)*3);
	  memcpy(&m[27], &m2[6], sizeof(float)*3);
	  memcpy(&m[30], &m3[6], sizeof(float)*3);
	  memcpy(&m[33], &m4[6], sizeof(float)*3);
}
...
Vec3 Math_MatrixMultMat3( Vec3 vert, float *matrix){
	  Vec3 out;
	  out.x = ((vert.x * matrix[0])  + (vert.y * matrix[1])  + (vert.z * matrix[2]));
	  out.y = ((vert.x * matrix[3])  + (vert.y * matrix[4])  + (vert.z * matrix[5]));
	  out.z = ((vert.x * matrix[6])  + (vert.y * matrix[7])  + (vert.z * matrix[8]));
	  return out;
}
